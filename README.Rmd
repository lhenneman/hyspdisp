...
Because it will typically be of interest to run ```hyspdisp``` for all of the emission events in ```date_ref_h```, ```hyspdisp_fac_model_parallel``` provides syntax to allow for parallelization:
```{r}
library( parallel)
