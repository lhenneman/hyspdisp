...
#======================================================================#
calc_grid_exposure <- function(year.E,
                               year.H,
                               pollutant = 'SO2..tons.',
                               units.mo,
                               rda_file = 'loaded',
  ...
  for (i in 1:12){
    ...
    month_mapping_long <- melt(month_mapping, id.vars = c('x', 'y'),
    ...
    if( time.agg == 'year') {
      ...
      ZIPexposures <- data.table(rbind(ZIPexposures,
                                       PP.linkage[, list(Exposure = sum(Exposure, na.rm = T)),
                                                  by = sum.by]))

      # sum over the year so far
      ZIPexposures <- ZIPexposures[, list(Exposure = sum(Exposure, na.rm = T)),
                                   by = sum.by]
    } else {
      # define aggregation strings
      if( source.agg == 'total')
        sum.by <- c( 'x', 'y', 'yearmonth')
      if( source.agg == 'facility')
        sum.by <- c( 'x', 'y', 'FacID', 'yearmonth')
      if( source.agg == 'unit')
        sum.by <- c( 'x', 'y', 'uID', 'yearmonth')
